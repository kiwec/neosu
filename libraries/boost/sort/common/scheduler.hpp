...
#include "boost/sort/common/noexcept_hlp.hpp"
...
struct scheduler
{
    ...
    void insert(Func_t & f)
    {
        ...
        if (itmp == mp.end())
        {
            ...
            if (aux.second == false) BOOST_TERMINATE("out of memory");
            itmp = aux.first;
        };
    ...
    void insert_range(it_func first, it_func last)
    {
        ...
        static_assert (std::is_same_v< Func_t, value2_t >,
        ...
        {
            auto aux = mp.emplace(th_id, deque_t());
            if (aux.second == true) BOOST_TERMINATE("out of memory");
            itmp = aux.first;
        };
        while (first != last)
        {
