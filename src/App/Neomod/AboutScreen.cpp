...
// reflow a run of body lines, advancing the layout cursor (height is doubled to fake line spacing)
void layoutLines(const std::vector<CBaseUIButton *> &lines, float &yCounter) {
    const float dpiScale = Osu::getUIScale();

    for(CBaseUIButton *line : lines) {
        line->onResized();  // HACKHACK: framework, setSizeToContent() does not update string metrics
        line->setSizeToContent();
        line->setSizeY(line->getSize().y * 2.0f);
        yCounter += line->getSize().y /* + 13 * dpiScale*/;
        line->setRelPos(35 * dpiScale, yCounter);
    }
}
...
AboutScreen::AboutScreen() : ScreenBackable() {
    ...
    // the pages themselves are filled in lazily, on first selection of their tab
    const auto makeTab = [this](Tab tab, const std::string &name, std::string text) {
        TabPage &page = this->tabs[static_cast<size_t>(tab)];

        page.view = new CBaseUIScrollView(0, 0, 0, 0, name + "View");
        page.view->setVerticalScrolling(true);
        page.view->setHorizontalScrolling(true);
        page.view->setDrawBackground(false);
        page.view->setDrawFrame(false);
        page.view->setScrollResistance(0);
        page.view->setVisible(false);
        this->addBaseUIElement(page.view);

        // added after the pages, so it draws (and hit-tests) above them
        page.button = new UIButtonRounded(0, 0, 0, 0, name + "Tab", std::move(text), 5);
        page.button->setDrawShadow(true)->setDrawBackground(false);
        page.button->setDrawsOnTop(true);
        page.button->setClickCallback([this, tab] { this->setActiveTab(tab); });
        this->addBaseUIElement(page.button);
    };

    makeTab(Tab::CHANGELOG, "AboutScreen::changelog", _("Changelog"));
    makeTab(Tab::CREDITS, "AboutScreen::credits", _("Credits"));
    makeTab(Tab::LICENSES, "AboutScreen::licenses", _("Licenses"));
}
...
CBaseUIContainer *AboutScreen::setVisible(bool visible) {
    ...
    // always open on the changelog, setActiveTab() does the (lazy) build + layout
    if(this->bVisible) this->setActiveTab(Tab::CHANGELOG);

    return this;
}

void AboutScreen::updateInput(CBaseUIEventCtx &c) {
    if(!this->bVisible) return;

    // registered FIRST = lowest wheel priority in this group, so the active tab's scrollview gets
    // first refusal and this only catches wheels over the tab strip
...
void AboutScreen::setActiveTab(Tab tab) {
    ...
    this->buildTab(tab);
    this->updateLayout();  // strip + page geometry, then a reflow of the (now built) active page
...
void AboutScreen::buildTab(Tab tab) {
    TabPage &page = this->tabs[static_cast<size_t>(tab)];
    if(page.built) return;
    page.built = true;

    // the header uses the same text as the tab button, and scrolls away with the content
    page.header = new CBaseUILabel(0, 0, 0, 0, "", std::string{page.button->getText()});
    page.header->setFont(osu->getTitleFont());
    page.header->setSizeToContent(0, 0);
    page.header->setDrawFrame(false);
    page.header->setDrawBackground(false);
    page.view->container.addBaseUIElement(page.header);
    ...
    switch(tab) {
        case Tab::CHANGELOG:
            ...
            break;
        case Tab::CREDITS:
            this->buildTextLines(page, "credits");
            break;
        case Tab::LICENSES:
...
void AboutScreen::buildTextLines(TabPage &page, std::string_view embedKey) {
    ...
    for(auto &line : textLines) {
        ...
        page.lines.push_back(btn);
        page.view->container.addBaseUIElement(btn);
    }
}

void AboutScreen::addAllChangelogs(std::vector<CHANGELOG> &&logtexts) {
    TabPage &page = this->tabs[static_cast<size_t>(Tab::CHANGELOG)];
    ...
    for(int i = 0; i < changelogs.size(); i++) {
        ...
        page.view->container.addBaseUIElement(changelog.title);
        ...
        for(auto &&changeText : changelogs[i].changes) {
            ...
            page.view->container.addBaseUIElement(change);
        }
...
void AboutScreen::updateLayout() {
    ...
    // equal width tabs, sized to the longest label so translations still fit
    float tabWidth = 0.0f;
    for(TabPage &page : this->tabs) {
        page.button->onResized();  // HACKHACK: framework, setWidthToContent() does not update string metrics
        page.button->setWidthToContent(static_cast<int>(20 * dpiScale));
        tabWidth = std::max(tabWidth, page.button->getSize().x);
    }
    tabWidth = std::min(tabWidth, (this->getSize().x - 2 * margin) / static_cast<f32>(NUM_TABS));

    for(size_t i = 0; i < NUM_TABS; i++) {
        this->tabs[i].button->setSize(tabWidth - 1, stripHeight);
        this->tabs[i].button->setRelPos(this->getSize().x - (margin + static_cast<f32>(NUM_TABS - i) * (tabWidth + 1)),
                                        margin);

        // the pages start below the strip, so scrolled content gets clipped by it instead of
        // drawn underneath it
        this->tabs[i].view->setRelPos(0, stripBottom);
        this->tabs[i].view->setSize(this->getSize().x, static_cast<f32>(osu->getVirtScreenHeight()) + 1 - stripBottom);
    }

    this->update_pos();  // setRelPos() only touches the relative rect

    // only the visible page needs to be in shape, the others get reflowed when they're selected
    this->layoutTab(this->activeTab);
}

void AboutScreen::layoutTab(Tab tab) {
    TabPage &page = this->tabs[static_cast<size_t>(tab)];
    if(!page.built) return;
    ...
    page.header->onResized();  // HACKHACK: framework, setSizeToContent() does not update string metrics
    page.header->setSizeToContent(0, 0);
    page.header->setRelPos(15 * dpiScale, yCounter);
    yCounter += page.header->getSize().y + 20 * dpiScale;
    ...
    if(tab == Tab::CHANGELOG) {
        for(const CHANGELOG_UI &changelog : this->changelogs) {
            ...
            layoutLines(changelog.changes, yCounter);
...
void AboutScreen::onChangeClicked(CBaseUIButton *button) {
    ...
    const auto maybeURLBeginIndex = changeTextMaybeContainingClickableURL.find("http");
...
// NOTE: kept at the bottom of the file, it's just a wall of text
void AboutScreen::buildChangelog() {
    ...
    v43_13.changes = {
        ...
        R"(- Fixed spinner skinning issues, mostly)",
    ...
    changelogs.push_back(std::move(v43_12));
    ...
    changelogs.push_back(std::move(v43_11));
    ...
    changelogs.push_back(std::move(v43_10));
    ...
    changelogs.push_back(std::move(v43_09));
    ...
    changelogs.push_back(std::move(v43_08));
    ...
    changelogs.push_back(std::move(v43_07));
    ...
    changelogs.push_back(std::move(v43_06));
    ...
    changelogs.push_back(std::move(v43_05));
    ...
    changelogs.push_back(std::move(v43_04));
    ...
    changelogs.push_back(std::move(v43_03));
    ...
    changelogs.push_back(std::move(v43_02));
    ...
    changelogs.push_back(std::move(v43_01));
    ...
    changelogs.push_back(std::move(v43_00));
    ...
    changelogs.push_back(std::move(v42_01));
    ...
    changelogs.push_back(std::move(v42_00));
    ...
    changelogs.push_back(std::move(v41_14));
    ...
    changelogs.push_back(std::move(v41_13));
    ...
    changelogs.push_back(std::move(v41_12));
    ...
    changelogs.push_back(std::move(v41_11));
    ...
    changelogs.push_back(std::move(v41_10));
    ...
    changelogs.push_back(std::move(v41_09));
    ...
    changelogs.push_back(std::move(v41_08));
    ...
    changelogs.push_back(std::move(v41_07));
    ...
    changelogs.push_back(std::move(v41_06));
    ...
    changelogs.push_back(std::move(v41_05));
    ...
    changelogs.push_back(std::move(v41_04));
    ...
    changelogs.push_back(std::move(v41_03));
    ...
    changelogs.push_back(std::move(v41_02));
    ...
    changelogs.push_back(std::move(v41_01));
    ...
    changelogs.push_back(std::move(v41_00));
    ...
    changelogs.push_back(std::move(v40_03));
    ...
    changelogs.push_back(std::move(v40_02));
    ...
    changelogs.push_back(std::move(v40_01));
    ...
    changelogs.push_back(std::move(v40_00));
    ...
    changelogs.push_back(std::move(v39_02));
    ...
    changelogs.push_back(std::move(v39_01));
    ...
    changelogs.push_back(std::move(v39_00));
    ...
    changelogs.push_back(std::move(v37_03));
    ...
    changelogs.push_back(std::move(v37_02));
    ...
    changelogs.push_back(std::move(v37_01));
    ...
    changelogs.push_back(std::move(v37_00));
    ...
    changelogs.push_back(std::move(v36_06));
    ...
    changelogs.push_back(std::move(v36_05));
    ...
    changelogs.push_back(std::move(v36_04));
    ...
    changelogs.push_back(std::move(v36_03));
    ...
    changelogs.push_back(std::move(v36_02));
    ...
    changelogs.push_back(std::move(v36_01));
    ...
    changelogs.push_back(std::move(v36_00));
    ...
    changelogs.push_back(std::move(v35_09));
    ...
    changelogs.push_back(std::move(v35_08));
    ...
    changelogs.push_back(std::move(v35_07));
    ...
    changelogs.push_back(std::move(v35_06));
    ...
    changelogs.push_back(std::move(v35_05));
    ...
    changelogs.push_back(std::move(v35_04));
    ...
    changelogs.push_back(std::move(v35_03));
    ...
    changelogs.push_back(std::move(v35_02));
    ...
    changelogs.push_back(std::move(v35_01));
    ...
    changelogs.push_back(std::move(v35_00));
    ...
    changelogs.push_back(std::move(v34_10));
    ...
    changelogs.push_back(std::move(v34_09));
    ...
    changelogs.push_back(std::move(v34_08));
    ...
    changelogs.push_back(std::move(v34_07));
    ...
    changelogs.push_back(std::move(v34_06));
    ...
    changelogs.push_back(std::move(v34_05));
    ...
    changelogs.push_back(std::move(v34_04));
    ...
    changelogs.push_back(std::move(v34_03));
    ...
    changelogs.push_back(std::move(v34_02));
    ...
    changelogs.push_back(std::move(v34_01));
    ...
    changelogs.push_back(std::move(v34_00));
