...
void load_and_watch(FinishedScore score) {
    ...
    if(score.replay.empty()) {
        osu->getNotificationOverlay()->addToast(ULITERAL("Failed to load replay"), ERROR_TOAST);
        return;
    }
    ...
    if(map == nullptr) {
        ...
        osu->getNotificationOverlay()->addToast(ULITERAL("Missing beatmap for this replay"), ERROR_TOAST);
    } else {
