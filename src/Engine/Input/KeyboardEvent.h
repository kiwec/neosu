...
class KeyboardEvent {
   public:
    KeyboardEvent(SCANCODE scanCode, uint64_t timestamp, bool repeat = false)
        : timestamp(timestamp), scanCode(scanCode), bRepeat(repeat) {}
    ...
    [[nodiscard]] constexpr forceinline char16_t getCharCode() const { return this->scanCode; }  // these are equivalent
