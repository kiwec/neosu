...
// _s for "safe"
template <typename T>
static inline bool strto_s(const std::string_view str, T& inout) {
    ...
    if constexpr(std::is_same_v<T, bool>) {
        ...
        reterr = ec;
        inout = (temp > 0) && ec == std::errc();
    } else if constexpr(std::is_same_v<T, uint8_t>) {
        uint32_t b{};
        auto [ptr, ec] = std::from_chars(begin, end, b);
        if(ec != std::errc() && b > 255) {
            inout = static_cast<uint8_t>(b);
        }
        reterr = ec;
    } else {
        auto [_, ec] = std::from_chars(begin, end, inout);
        if(ec != std::errc()) {
            inout = T{};
        }
        reterr = ec;
